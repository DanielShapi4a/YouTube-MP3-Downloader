...
const download = (sourceUrl, destination, redirectCount = 0) => new Promise((resolve, reject) => {
  if (redirectCount > 5) {
  ...
  https.get(sourceUrl, (response) => {
    if ([301, 302, 303, 307, 308].includes(response.statusCode || 0) && response.headers.location) {
      response.resume();
      download(new URL(response.headers.location, sourceUrl).toString(), destination, redirectCount + 1)
        .then(resolve)
        .catch(reject);
      return;
    }
