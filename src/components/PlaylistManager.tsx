  ...
  return (
    <div className={`flex-1 flex flex-col gap-6 p-6 ${isRtl ? 'text-right' : 'text-left'}`}>
      ...
      <section className="flex flex-col md:flex-row items-stretch md:items-end justify-between gap-4 shrink-0">
        <div>
          <h2 className="font-headline-lg text-headline-lg text-on-background font-extrabold">
            {t.playlistQueue}
          </h2>
      ...
      {isInputOpen && (
        <form
          ...
          className="bg-surface-container-low border border-secondary/30 rounded-xl p-5 shrink-0 flex flex-col gap-3 animate-[fadeIn_0.2s_ease-out]"
        >
          <div className="flex flex-col gap-1">
            <label className="text-xs font-label-bold text-secondary uppercase tracking-wider">
              {t.inputPlaylistUrlPlaceholder}
            </label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {playlists.map((playlist) => {
              const percentage = Math.round(
                (playlist.downloadedTracks / playlist.totalTracks) * 100,
              );
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className="absolute bottom-3 left-4 right-4 flex items-center gap-2 z-10"
                      style={{ direction: isRtl ? 'rtl' : 'ltr' }}
                    >
                      <div className="w-7 h-7 rounded bg-secondary/20 border border-secondary/30 shrink-0 flex items-center justify-center">
                        <span className="material-symbols-outlined text-[15px] text-secondary font-black">
                          play_arrow
                        </span>
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <span
                          className="text-[11px] text-white font-extrabold truncate block mt-0.5"
                          title={
                    ...
                    <div className="mt-auto">
                      <div className="flex justify-between items-end mb-2 text-xs">
                        <span className="font-label-bold text-xs text-secondary-fixed font-semibold whitespace-nowrap">
                          {playlist.downloadedTracks} / {playlist.totalTracks}{' '}
                          {settings.language === Language.HE
                            ? 'שירים'
                            : settings.language === Language.RU
                              ? 'Треков'
                  ...
                  <div className="px-4 py-2.5 border-t border-surface-container-highest bg-surface-dim/40 flex gap-2 shrink-0">
                    {!isCompleted ? (
                      isProcessing ? (
                        ...
                        >
                          <span
                            className="material-icons-span text-sm pr-0.5"
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-surface-container-high bg-surface-dim shrink-0">
              <div className="flex items-center gap-3">
                <span className="material-icons-span text-secondary text-2xl font-black">
                  music_video
                </span>
                <div>
                  ...
                  <span className="text-[10px] text-on-surface-variant mt-1.5 block opacity-85 leading-none font-mono">
                    {selectedPlaylistForTracks.totalTracks}{' '}
                    {settings.language === Language.HE
                      ? 'שירים בסך הכל'
                      : settings.language === Language.RU
                        ? 'Всего треков'
            ...
            <div className="p-4 overflow-y-auto space-y-2">
              {selectedPlaylistForTracks.tracks && selectedPlaylistForTracks.tracks.length > 0 ? (
                ...
                <div className="text-center py-8 text-on-surface-variant/60">
                  <span className="material-icons-span text-3xl opacity-50 block mb-1">
