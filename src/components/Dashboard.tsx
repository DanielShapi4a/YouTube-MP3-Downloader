...
export default function Dashboard({
  ...
  const quickSamples = [
    { title: "Rick Astley (Pop)", url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ", mode: "single" },
    { title: "M83 Midnight (Indie)", url: "https://www.youtube.com/watch?v=T8B0E8l8gYo", mode: "single" },
    { title: "Kavinsky Nightcall (Synth)", url: "https://www.youtube.com/watch?v=MV_3Dpw-BRY", mode: "single" },
    { title: "Lo-Fi Beats (Playlist)", url: "https://www.youtube.com/playlist?list=PL_lofi_study_beats", mode: "playlist" },
    { title: "Synthwave Car (Playlist)", url: "https://www.youtube.com/playlist?list=PL_retro_car_synth", mode: "playlist" },
  ];
    ...
    <div className={`flex-1 flex flex-col gap-6 p-6 ${isRtl ? 'text-right' : 'text-left'}`}>
      ...
      <section className="flex flex-col gap-5 bg-surface-container-low border border-outline-variant/30 rounded-xl p-6 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-1">
          <h2 className="font-headline-lg text-headline-lg text-on-surface font-extrabold">{t.newDownload}</h2>
        ...
        <div className="flex flex-col gap-2 mt-1">
          <label className="text-xs font-label-bold text-on-surface-variant uppercase tracking-wider">{t.downloadType}</label>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center bg-surface-dim border border-outline-variant/40 rounded-lg p-2 input-focus-glow transition-all duration-200">
            ...
            <input 
              ...
              placeholder={downloadMode === 'playlist' ? "https://www.youtube.com/playlist?list=..." : "https://www.youtube.com/watch?v=..."}
              type="text" 
              value={urlInput}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] text-on-surface-variant uppercase font-label-bold">Samples:</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 bg-surface-container-highest/20 p-4 rounded-lg border border-outline-variant/20 mt-1">
            ...
            <div className="flex flex-col gap-1 flex-1">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center w-8 h-8 rounded bg-secondary/10 text-secondary shrink-0">
                  <span className="material-icons-span text-lg" style={{ fontVariationSettings: "'FILL' 1" }}>folder</span>
                </div>
                <span className="font-mono text-xs text-on-surface truncate pr-2 max-w-[240px] block font-semibold">{settings.saveLocation}</span>
              </div>
            </div>
            ...
            <button 
              ...
              className={`flex items-center justify-center gap-2 bg-gradient-to-b from-primary-container to-error text-on-primary-container px-6 py-3 rounded-lg font-label-bold text-sm border-t border-white/20 primary-glow transition-all duration-200 select-none whitespace-nowrap ${
                activeDownload ? 'opacity-40 cursor-not-allowed' : 'hover:opacity-90 active:scale-95 cursor-pointer'
              }`}
            >
              <span className="material-icons-span text-lg" style={{ fontVariationSettings: "'FILL' 1" }}>download</span>
      ...
      <section className="flex flex-col gap-2">
        <h3 className="font-label-bold text-xs text-on-surface-variant uppercase tracking-wider pl-1">{t.activeDownload}</h3>
        {activeDownload ? (
          <div id="active-download-card" className="bg-surface-container-highest border border-outline-variant/30 rounded-xl p-4 flex flex-col md:flex-row items-stretch md:items-center gap-4 relative overflow-hidden shadow-lg animate-[fadeIn_0.3s_ease]">
            ...
            <div className="flex-1 flex flex-col gap-1 z-10 min-w-0">
              <div className="flex justify-between items-start">
                <h4 className="font-body-lg text-sm text-on-surface font-bold truncate pr-4">{activeDownload.title}</h4>
                <span className="font-mono text-sm text-secondary-fixed-dim whitespace-nowrap font-bold">{activeDownload.progress}%</span>
              </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded bg-secondary-container/10 text-secondary border border-secondary-container/20 text-[10px] uppercase font-label-bold tracking-wider">
                    {activeDownload.status === 'converting' 
                      ? (settings.language === Language.HE ? "מקודד" : settings.language === Language.RU ? "Конвертация" : "Converting")
                      : (settings.language === Language.HE ? "מוריד" : settings.language === Language.RU ? "Загрузка" : "Downloading")
                    }
                  </span>
                  <span className="text-on-surface-variant font-medium">{activeDownload.speed}</span>
                </div>
                <span className="text-on-surface-variant font-medium">{t.eta}: {activeDownload.eta}</span>
              </div>
            </div>
            ...
            <div className="flex items-center z-10 pl-2 shrink-0 border-outline-variant/20 md:border-l border-t md:border-t-0 pt-3 md:pt-0">
              ...
              >
                ...
                <span className="text-[9px] font-label-bold text-on-surface-variant uppercase tracking-wider">{t.cancel}</span>
              </button>
            </div>
          ...
          <div className="border border-dashed border-outline-variant/20 rounded-xl p-8 text-center flex flex-col items-center justify-center bg-surface-container-low/30 text-on-surface-variant/40 select-none">
            ...
            <span className="text-xs font-label-bold uppercase tracking-widest">No Active Downloads</span>
          </div>
        )}
      </section>
      ...
      <section className="mt-auto border-t border-outline-variant/20 pt-4 flex flex-col gap-2">
        <div className="flex justify-between items-center">
          ...
          >
            ...
            {t.advancedLogs}
            <span className="material-icons-span text-sm pr-1">{isLogsOpen ? "expand_less" : "expand_more"}</span>
          </button>
          ...
          {isLogsOpen && (
            ...
            >
              {settings.language === Language.HE ? "נקה לוג" : settings.language === Language.RU ? "Очиститьлог" : "Clear Log"}
            </button>
          )}
        </div>

        {isLogsOpen && (
          <div id="logs-container-panel" className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-3 font-mono text-[11px] h-36 overflow-y-auto flex flex-col gap-1 select-text">
            {logs.length > 0 ? (
              [...logs].reverse().map((log) => (
                <div key={log.id} className="flex items-start gap-2 leading-relaxed">
                  <span className="text-on-surface-variant select-none opacity-40 shrink-0">[{log.timestamp}]</span>
                  <span className={`font-bold shrink-0 ${
                    log.type === 'success' ? 'text-secondary-fixed-dim' :
                    log.type === 'error' ? 'text-error' :
                    log.type === 'warning' ? 'text-primary' : 'text-blue-400'
                  }`}>
              ...
              <span className="text-on-surface-variant/30 text-center py-8">Logs are empty. Engine running perfectly...</span>
            )}
          </div>
        )}
      </section>
