    ...
    <header className="bg-surface/95 border-b border-outline-variant/30 sticky top-0 md:h-16 h-14 w-full flex justify-between items-center px-4 shrink-0 z-40 backdrop-blur-md">
      ...
      <div className={`flex items-center gap-2 ${isRtl ? 'order-last' : ''}`}>
        ...
        <div className="relative flex items-center">
          ...
          >
            <option value={Language.EN} className="bg-surface text-on-surface">
              English
            </option>
            <option value={Language.RU} className="bg-surface text-on-surface">
              Русский
            </option>
            <option value={Language.HE} className="bg-surface text-on-surface">
              עברית
            </option>
          </select>
          <span className="material-icons-span text-on-surface-variant text-[14px] pointer-events-none absolute right-0 top-1/2 -translate-y-1/2">
            expand_more
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 text-on-surface-variant">
        ...
        >
          ...
          <span className="font-label-bold text-xs font-semibold select-none hidden sm:inline">
            {t.openFolder}
          </span>
        </button>
